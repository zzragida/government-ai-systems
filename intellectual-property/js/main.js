...
const API_URL = '/api/intellectual-property';
...
function App() {
    ...
    const tabs = [
        ...
        { id: 'layers', label: '🏗️ 5계층' },
        { id: 'verify', label: '🔐 검증' },
        { id: 'patent', label: '🤖 AI 특허 출원' }
    ];
    
    return React.createElement('div', { className: 'min-h-screen bg-gray-50' },
        React.createElement('header', { className: 'bg-gradient-to-r from-blue-600 to-indigo-700 text-white p-6 shadow-xl' },
                    ...
                    tabs.map(t =>
                        React.createElement('button', {
                            ...
                            className: `px-6 py-3 rounded-lg font-semibold whitespace-nowrap transition ${
                                tab === t.id ? 'bg-blue-600 text-white shadow-lg' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                            }`
                        }, t.label)
                    )
                )
        ...
        React.createElement('main', { className: 'container mx-auto p-8' },
            tab === 'home' && React.createElement(HomeTab),
            tab === 'scenarios' && React.createElement(ScenariosTab),
            tab === 'consultation' && React.createElement(ConsultationTab),
            tab === 'layers' && React.createElement(LayersTab),
            tab === 'verify' && React.createElement(VerifyTab),
            tab === 'patent' && React.createElement(PatentFilingTabComplete)
        )
    );
}
...
function HomeTab() {
    return React.createElement('div', { className: 'space-y-6' },
        React.createElement('div', { className: 'bg-gradient-to-r from-purple-600 to-indigo-700 rounded-xl shadow-xl p-8 text-white' },
            ...
            React.createElement('p', { className: 'text-lg' }, '오픈해시 분산 신뢰 시스템과 DeepSeek R1 AI를 결합한 차세대 플랫폼')
        ),
        React.createElement('div', { className: 'grid md:grid-cols-3 gap-6' },
            ...
            React.createElement('div', { className: 'bg-white rounded-xl shadow-lg p-6 text-center' },
                React.createElement('div', { className: 'text-4xl mb-2' }, '🌐'),
                React.createElement('div', { className: 'text-3xl font-bold text-purple-600 mb-2' }, '280,000개'),
...
function ScenariosTab() {
    ...
    useEffect(() => {
        fetch(`${API_URL}/scenarios`).then(r => r.json()).then(d => setData(d.scenarios || [])).catch(e => console.error(e));
    }, []);
    return React.createElement('div', { className: 'space-y-6' },
        React.createElement('h2', { className: 'text-2xl font-bold mb-4' }, '💡 실제 문제 해결 시나리오'),
        data.length === 0 ? React.createElement('p', {}, '로딩 중...') : data.map((s, i) =>
            React.createElement('div', { key: i, className: 'bg-white rounded-xl shadow-lg p-6' },
                ...
                React.createElement('div', { className: 'bg-red-50 border-l-4 border-red-500 p-4 rounded-r-lg mb-3' },
                    React.createElement('p', { className: 'font-bold text-red-800 mb-2' }, '❌ 기존 문제'),
                    React.createElement('p', {}, s.problem)
                ),
                React.createElement('div', { className: 'bg-green-50 border-l-4 border-green-500 p-4 rounded-r-lg' },
                    React.createElement('p', { className: 'font-bold text-green-800 mb-2' }, '✅ AI 해결'),
                    React.createElement('p', {}, s.solution)
                )
            )
        )
    );
...
function ConsultationTab() {
    ...
    const [selected, setSelected] = useState('applicant_consultant');
    const [msg, setMsg] = useState('');
    const [reply, setReply] = useState('');
    const [loading, setLoading] = useState(false);
    useEffect(() => {
        fetch(`${API_URL}/agents`).then(r => r.json()).then(d => setAgents(d.agents || [])).catch(e => console.error(e));
    }, []);
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await fetch(`${API_URL}/consultation`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ message: msg, agent_type: selected })
            });
            const data = await res.json();
            setReply(data.response);
        } catch (err) {
            setReply('오류: ' + err.message);
        } finally {
            setLoading(false);
        }
    };
    return React.createElement('div', { className: 'max-w-4xl mx-auto' },
        React.createElement('div', { className: 'bg-white rounded-xl shadow-lg p-8' },
            ...
            reply && React.createElement('div', { className: 'mt-6 bg-blue-50 border-l-4 border-blue-500 p-6 rounded-r-lg' },
                React.createElement('p', { className: 'font-bold text-blue-800 mb-2' }, '🤖 AI 응답'),
                React.createElement('p', { className: 'whitespace-pre-wrap' }, reply)
            )
        )
    );
}
...
function VerifyTab() {
    return React.createElement('div', { className: 'max-w-5xl mx-auto bg-white rounded-xl shadow-lg p-8' },
        React.createElement('h2', { className: 'text-3xl font-bold mb-6' }, '🔐 오픈해시 문서 검증')
    );
}
...
function PDVModal({ onClose }) {
    return React.createElement('div', {
        ...
        React.createElement('div', {
            ...
            React.createElement('div', { className: 'bg-green-600 text-white p-6 flex justify-between' },
                React.createElement('h3', { className: 'text-2xl font-bold' }, '✅ PDV 금고'),
                React.createElement('button', { onClick: onClose, className: 'text-3xl' }, '×')
            ),
            React.createElement('div', { className: 'p-8' },
                React.createElement('p', {}, 'PDV 금고 내용')
            )
        )
    );
}

function HashModal({ onClose }) {
    return React.createElement('div', {
        className: 'fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4',
        onClick: onClose
    },
        React.createElement('div', {
            ...
            React.createElement('div', { className: 'bg-indigo-600 text-white p-6 flex justify-between' },
                React.createElement('h3', { className: 'text-2xl font-bold' }, '🌐 오픈해시'),
                React.createElement('button', { onClick: onClose, className: 'text-3xl' }, '×')
            ),
            React.createElement('div', { className: 'p-8' },
                React.createElement('p', {}, '오픈해시 내용')
            )
        )
    );
}
...
function MultiCountryModal({ onClose }) {
    return React.createElement('div', {
        ...
        React.createElement('div', {
            ...
            React.createElement('div', { className: 'bg-purple-600 text-white p-6 flex justify-between' },
                React.createElement('h3', { className: 'text-2xl font-bold' }, '🌍 다국가 평가'),
                React.createElement('button', { onClick: onClose, className: 'text-3xl' }, '×')
            ),
            React.createElement('div', { className: 'p-8' },
                React.createElement('p', {}, '다국가 평가 내용')
            )
        )
    );
}

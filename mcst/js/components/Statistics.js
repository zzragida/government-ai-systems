const Statistics = () => {
    ...
    return (
        <div className="space-y-6">
            <div className="bg-gradient-to-r from-pink-700 to-rose-700 text-white rounded-lg shadow-lg p-8">
                    ...
                    {efficiencyMetrics.map((item, idx) => (
                        <div key={idx} className="border border-gray-200 rounded-lg p-4">
                            <div className="flex items-center justify-between mb-2">
                                ...
                                <span className="text-lg font-bold text-pink-600">{item.improvement.startsWith('+') ? item.improvement : `-${item.improvement}`}</span>
                            </div>
                            <div className="grid grid-cols-2 gap-4 text-sm">
                                <div>
                                    <div className="text-gray-500 mb-1">AI 도입 전</div>
                                    <div className="font-semibold text-pink-600">{item.before}</div>
                                </div>
                                <div>
                                    <div className="text-gray-500 mb-1">AI 도입 후</div>
                                    <div className="font-semibold text-pink-600">{item.after}</div>
                                </div>
                            </div>
                        </div>
                    ))}

...
const AIChat = () => {
    const [messages, setMessages] = useState([
        {
            role: 'assistant',
            content: '안녕하세요! 국세청 AI 상담 시스템입니다. 세무 관련 궁금하신 점을 질문해주세요.'
        }
    ]);
    ...
    const handleSend = async () => {
        ...
        // 시뮬레이션: 실제로는 /api/nts/chat 엔드포인트 호출
        setTimeout(() => {
            const aiResponse = {
                role: 'assistant',
                content: `"${input}"에 대한 답변입니다. 실제 운영 시에는 Claude API를 통해 정확한 세무 상담을 제공합니다. 국가데이터처에서 관련 법령과 사례를 인출하여 답변합니다.`
            };
    ...
    return (
        <div className="space-y-6">
            <div className="bg-gradient-to-r from-indigo-600 to-purple-700 text-white rounded-lg shadow-lg p-8">
                <h2 className="text-3xl font-bold mb-4">AI 세무 상담</h2>
                <p className="text-lg text-indigo-100">
                    Claude API 기반 실시간 세무 상담 서비스입니다.
            ...
            <div className="bg-white rounded-lg shadow-md overflow-hidden">
                ...
                <div className="border-t border-gray-200 p-4 bg-white">
                    ...
                    <p className="text-xs text-gray-500 mt-2">
                        ⚠️ 본 AI 상담은 참고용이며, 정확한 세무 상담은 전문가와 상담하시기 바랍니다.
                    </p>
                </div>
            </div>
        </div>

    ...
    return (
        <div className="space-y-6">
            <div className="bg-white rounded-lg shadow-md p-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">실시간 해양 경비 로그</h2>
                <p className="text-sm text-gray-600">
                    모든 해양경찰관의 국가데이터처 접근 기록과 업무 처리 내역을 실시간으로 추적합니다.
                </p>
            </div>
...
function generateSampleLogs(count) {
    const departments = ['서해지방해양경찰청 경비과', '남해지방해양경찰청 구조과', '경비국 작전과', '수사정보국 수사과', '해양안전국 안전관리과'];
    const names = ['김해양', '이경비', '박구조', '정단속', '최수색', '강방제', '윤안전', '한사고'];
    const positions = ['경정', '경감', '경위', '경사', '순경'];
    const taskTypes = ['해상경비', '인명구조', '범죄수사', '안전점검', '오염방제'];
    const targets = ['122 신고 대응', '조난선박 구조', '밀수 단속', '어선 안전점검', '기름유출 방제'];
    ...
    for (let i = 0; i < count; i++) {
        ...
        logs.push({
            logId: `KCG-${Date.now()}-${i}`,
            timestamp: timestamp.toISOString(),
            employee: {
                id: `EMP-KCG-${String(i+1).padStart(3, '0')}`,
                name: names[Math.floor(Math.random() * names.length)],
                department: departments[Math.floor(Math.random() * departments.length)],
                position: positions[Math.floor(Math.random() * positions.length)],
                grade: positions[Math.floor(Math.random() * positions.length)]
            },
            task: {
                type: taskTypes[Math.floor(Math.random() * taskTypes.length)],
                caseNumber: `MARINE-2025-${String(Math.floor(Math.random() * 9999)).padStart(4, '0')}`,
                targetEntity: targets[Math.floor(Math.random() * targets.length)],
                purpose: '해양 안전 및 치안 확립'
            },
            ndrOperation: {
                operationType: operationType,
                dataCategory: '해양_경비기록',
                dataPath: `/ndr/coastguard/operation/2025/${Math.floor(Math.random() * 1000)}`,
                recordCount: Math.floor(Math.random() * 600) + 200,
                dataSize: `${(Math.random() * 5 + 1).toFixed(1)}MB`,
                retrievedFields: ['신고내용', '출동기록', '구조현황', '단속내역']
            },
            openHash: {
            ...
            resultData: operationType === 'STORE' ? {
                operationType: 'STORE',
                dataCategory: '해양_대응결과',
                dataPath: `/ndr/coastguard/result/2025/MARINE-${String(Math.floor(Math.random() * 9999)).padStart(4, '0')}`,
                conclusion: `${['구조 완료', '단속 완료', '안전점검 완료', '방제 완료'][Math.floor(Math.random() * 4)]}`,

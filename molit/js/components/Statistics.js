const Statistics = () => {
    ...
    return (
        <div className="space-y-6">
            <div className="bg-gradient-to-r from-slate-700 to-gray-800 text-white rounded-lg shadow-lg p-8">
                ...
                <div className="h-64 flex items-end justify-around border-b border-l border-gray-300 pb-4 pl-4">
                    {performanceData.datasets[0].data.map((value, idx) => (
                        <div key={idx} className="flex flex-col items-center" style={{width: '12%'}}>
                            ...
                            <div 
                                className="w-full bg-gradient-to-t from-gray-600 to-blue-400 rounded-t"
                    ...
                    {efficiencyMetrics.map((item, idx) => (
                        <div key={idx} className="border border-gray-200 rounded-lg p-4">
                            <div className="flex items-center justify-between mb-2">
                                ...
                                <span className="text-lg font-bold text-gray-600">{item.improvement.startsWith('+') ? item.improvement : `-${item.improvement}`}</span>
                            </div>
                            <div className="grid grid-cols-2 gap-4 text-sm">
                                <div>
                                    <div className="text-gray-500 mb-1">AI 도입 전</div>
                                    <div className="font-semibold text-gray-600">{item.before}</div>
                                </div>
                                <div>
                                    <div className="text-gray-500 mb-1">AI 도입 후</div>
                                    <div className="font-semibold text-gray-600">{item.after}</div>
                                </div>
                            </div>
                        </div>
                    ))}

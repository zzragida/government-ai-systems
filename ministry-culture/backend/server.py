...
# 초기 블록 생성
add_to_chain('시스템 초기화', {'agency': '문화체육관광부', 'id': 'ministry-culture'})

@app.route('/health', methods=['GET'])
@app.route('/api/health', methods=['GET'])
def health():
    return jsonify({'status': 'ok', 'service': 'ministry-culture-ai-system', 'agency': '문화체육관광부'})
    ...
    try:
        response = requests.post(
            ...
            json={
                ...
                'max_tokens': 2048,
                'system': '''당신은 문화체육관광부의 AI 업무 보조 에이전트입니다.
                
주요 역할:
1. 문화체육관광부 관련 정책 및 업무 안내
2. 민원 상담 및 안내
3. 관련 법령 및 규정 설명
4. 다른 정부 부처와의 협업 지원
...
if __name__ == '__main__':
    print(f"🚀 문화체육관광부 AI 시스템 시작 (포트: 5036)")
